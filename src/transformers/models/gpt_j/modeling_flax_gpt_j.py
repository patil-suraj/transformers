...
from .configuration_gpt_j import GPTJConfig
...
### helpers from mesh-transformer-jax
...
class FlaxGPTJEmbed(nn.Module):
    ...
    def setup(self):
        ...
        self.bias = self.param("bias", jax.nn.initializers.zeros, (self.features,))
    ...
    def __call__(self, inputs):
        ...
        return jnp.take(self.embedding, inputs, axis=0) + self.bias
...
class FlaxGPTJAttention(nn.Module):
    ...
    def __call__(
        ...
        # rotary embeds
        k_rot = key[:, :, : self.config.pe_rotary_dims]
        k_pass = key[:, :, self.config.pe_rotary_dims :]

        q_rot = query[:, :, : self.config.pe_rotary_dims]
        q_pass = query[:, :, self.config.pe_rotary_dims :]
...
class FlaxGPTJBlock(nn.Module):
    ...
    def setup(self):
        ...
        intermediate_size = (
            self.config.intermediate_size if self.config.intermediate_size is not None else 4 * hidden_size
        )

        self.ln_1 = nn.LayerNorm(epsilon=self.config.layer_norm_epsilon, dtype=self.dtype)
        self.attn = FlaxGPTJAttention(self.config, dtype=self.dtype)
        self.mlp = FlaxGPTJMLP(self.config, intermediate_size, dtype=self.dtype)
...
append_call_sample_docstring(
    FlaxGPTJModel, _TOKENIZER_FOR_DOC, _CHECKPOINT_FOR_DOC, FlaxBaseModelOutput, _CONFIG_FOR_DOC
)


class FlaxGPTJLMHeadModule(nn.Module):
    config: GPTJConfig
    dtype: jnp.dtype = jnp.float32
...
class FlaxGPTJLMHeadModel(FlaxGPTJPreTrainedModel):
    module_class = FlaxGPTJLMHeadModule
...
append_call_sample_docstring(
    FlaxGPTJLMHeadModel, _TOKENIZER_FOR_DOC, _CHECKPOINT_FOR_DOC, FlaxCausalLMOutput, _CONFIG_FOR_DOC
)
