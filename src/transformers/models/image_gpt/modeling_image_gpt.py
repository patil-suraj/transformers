...
class ImageGPTLayerNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        variance = torch.square(hidden_states).mean(dim=-1, keepdim=True)
...
class ImageGPTBlock(nn.Module):
    def __init__(self, config):
        ...
        self.attn = ImageGPTAttention(config)
...
ImageGPT_INPUTS_DOCSTRING = r"""
    Args:
        ...
        past_key_values (:obj:`Tuple[Tuple[torch.Tensor]]` of length :obj:`config.num_layerss`):
...
PARALLELIZE_DOCSTRING = r"""
    ...
    Example::
            ...
            model = ImageGPTLMHeadModel.from_pretrained('gpt2-xl')
...
DEPARALLELIZE_DOCSTRING = r"""
    ...
    Example::
        ...
        model = ImageGPTLMHeadModel.from_pretrained('gpt2-large')
...
class ImageGPTModel(ImageGPTPreTrainedModel):
    def __init__(self, config):
        ...
        self.blocks = nn.ModuleList([ImageGPTBlock(config) for _ in range(config.num_layers)])

        self.ln_f = ImageGPTLayerNorm(self.embed_dim, eps=config.layer_norm_epsilon)

        self.bos = nn.Parameter(nn.init.normal_(torch.Tensor(self.embed_dim), std=0.02))
    ...
    def parallelize(self, device_map=None):
        ...
        self.device_map = (
            get_device_map(len(self.blocks), range(torch.cuda.device_count())) if device_map is None else device_map
        )
    ...
    def forward(
        ...
        # head_mask has shape num_layers x batch x n_heads x N x N
        ...
        bos_embeds = (
            torch.ones((batch_size, 1, self.embed_dim), device=hidden_states.device, dtype=hidden_states.dtype)
            * self.bos
        )
        hidden_states = torch.cat([bos_embeds + hidden_states[:, :-1, :]], dim=1)
