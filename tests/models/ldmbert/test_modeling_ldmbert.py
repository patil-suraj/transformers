...
""" Testing suite for the PyTorch LDMBERT model. """
...
from transformers import LDMBertConfig, is_torch_available
...
if is_torch_available():
    ...
    from transformers import (
        ...
        BartTokenizer,
        LDMBertForCausalLM,
        LDMBertForConditionalGeneration,
        LDMBertForQuestionAnswering,
        LDMBertForSequenceClassification,
        LDMBertModel,
        pipeline,
    )
    from transformers.models.ldmbert.modeling_ldmbert import LDMBertDecoder, LDMBertEncoder, shift_tokens_right


def prepare_ldmbert_inputs_dict(
    config,
    input_ids,
...
class LDMBertModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        inputs_dict = prepare_ldmbert_inputs_dict(config, input_ids, decoder_input_ids)
    ...
    def create_and_check_decoder_model_past_large_inputs(self, config, inputs_dict):
        model = LDMBertModel(config=config).get_decoder().to(torch_device).eval()
    ...
    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        model = LDMBertModel(config=config).to(torch_device).eval()
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = LDMBertEncoder.from_pretrained(tmpdirname).to(torch_device)
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = LDMBertDecoder.from_pretrained(tmpdirname).to(torch_device)
...
@require_torch
class LDMBertHeadTests(unittest.TestCase):
    vocab_size = 99
    ...
    def test_sequence_classification_forward(self):
        ...
        model = LDMBertForSequenceClassification(config)
    ...
    def test_question_answering_forward(self):
        ...
        model = LDMBertForQuestionAnswering(config)
        model.to(torch_device)
        outputs = model(
            input_ids=input_ids,
    ...
    def test_lm_forward(self):
        ...
        lm_model = LDMBertForConditionalGeneration(config)
    ...
    def test_lm_uneven_forward(self):
        ...
        lm_model = LDMBertForConditionalGeneration(config).to(torch_device)
    ...
    def test_tokenization(self):
        tokenizer = BartTokenizer.from_pretrained("facebook/ldmbert-large")
        ...
        for ex, desired_result in zip(examples, fairseq_results):
            ldmbert_toks = tokenizer.encode(ex, return_tensors="pt").squeeze()
            assert_tensors_close(desired_result.long(), ldmbert_toks, prefix=ex)

    def test_generate_fp16(self):
        config, input_ids, batch_size = self._get_config_and_data()
        attention_mask = input_ids.ne(1).to(torch_device)
        model = LDMBertForConditionalGeneration(config).eval().to(torch_device)
...
@require_torch
class LDMBertModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (
        (LDMBertModel, LDMBertForConditionalGeneration, LDMBertForSequenceClassification, LDMBertForQuestionAnswering)
        if is_torch_available()
        else ()
    )
    all_generative_model_classes = (LDMBertForConditionalGeneration,) if is_torch_available() else ()
    ...
    def setUp(self):
        self.model_tester = LDMBertModelTester(self)
        self.config_tester = ConfigTester(self, config_class=LDMBertConfig)
    ...
    # LDMBertForSequenceClassification does not support inputs_embeds
    def test_inputs_embeds(self):
        config, inputs_dict = self.model_tester.prepare_config_and_inputs_for_common()

        for model_class in (LDMBertModel, LDMBertForConditionalGeneration, LDMBertForQuestionAnswering):
            model = model_class(config)
            model.to(torch_device)
            model.eval()
...
class FastIntegrationTests(unittest.TestCase):
    ...
    @cached_property
    def tok(self):
        return BartTokenizer.from_pretrained("facebook/ldmbert-large")

    @cached_property
    def xsum_1_1_model(self):
        return LDMBertForConditionalGeneration.from_pretrained("sshleifer/distilldmbert-xsum-1-1")
...
@require_tokenizers
class LDMBertModelIntegrationTests(unittest.TestCase):
    @cached_property
    ...
    def test_inference_no_head(self):
        model = LDMBertModel.from_pretrained("facebook/ldmbert-large").to(torch_device)
    ...
    def test_base_mask_filling(self):
        pbase = pipeline(task="fill-mask", model="ldm-bert")
    ...
    def test_large_mask_filling(self):
        plarge = pipeline(task="fill-mask", model="facebook/ldmbert-large")
        src_text = [" I went to the <mask>."]
    ...
    def test_mnli_inference(self):
        ...
        model = AutoModelForSequenceClassification.from_pretrained("facebook/ldmbert-large-mnli").to(
            torch_device
    ...
    def test_xsum_summarization_same_as_fairseq(self):
        model = LDMBertForConditionalGeneration.from_pretrained("facebook/ldmbert-large-xsum").to(torch_device)
    ...
    def test_xsum_config_generation_params(self):
        config = LDMBertConfig.from_pretrained("facebook/ldmbert-large-xsum")
    ...
    def test_cnn_summarization_same_as_fairseq(self):
        hf = LDMBertForConditionalGeneration.from_pretrained("facebook/ldmbert-large-cnn").to(torch_device)
        tok = BartTokenizer.from_pretrained("facebook/ldmbert-large")
...
class LDMBertStandaloneDecoderModelTester:
    def __init__(
        self,
        parent,
        vocab_size=99,
    ...
    def create_and_check_decoder_model_past(
        ...
        model = LDMBertDecoder(config=config).to(torch_device).eval()
        # first forward pass
...
@require_torch
class LDMBertStandaloneDecoderModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (LDMBertDecoder, LDMBertForCausalLM) if is_torch_available() else ()
    all_generative_model_classes = (LDMBertForCausalLM,) if is_torch_available() else ()
    fx_comptatible = True
    test_pruning = False
    is_encoder_decoder = False
    ...
    def setUp(
        ...
        self.model_tester = LDMBertStandaloneDecoderModelTester(self, is_training=False)
        self.config_tester = ConfigTester(self, config_class=LDMBertConfig)
